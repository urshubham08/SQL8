...
-- diffrent between each order amount and the avg 

select order_id , category ,  amount , avg(amount) over (partition by category)  , round(amount - avg(amount) over (partition by category) ) from orders ;

select customer_name , order_id , max(amount) over (partition by order_id ) from orders ;
